...
fn filter_detail_sections(report: &mut JsonReport) {
    ...
    report.flutter_style.retain(|finding| {
        style_kind(&finding.kind)
            .is_some_and(|kind| detail_path_live(kind, &finding.path, &remaining))
    });
    report
        .hotspots
...
fn style_kind(kind: &str) -> Option<FindingKind> {
    match kind {
...
fn recompute_summary(report: &mut JsonReport) {
    ...
    report.summary.mixed_go_router_styles =
        kind_count(&report.findings, FindingKind::MixedGoRouterStyle);
